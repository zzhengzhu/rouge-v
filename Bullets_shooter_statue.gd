...
func on_bullet_spawned():
	...
	statue = get_parent().get_node("enemy").get_shooting_statue()
	...
	for i in BULLET_COUNT:
		...
		bullet.direction = (to_local(player.get_transform().origin) - get_parent().get_node("enemy").get_transform().origin).normalized()
		...
		PhysicsServer2D.body_set_collision_mask(bullet.body, 2)
...
func _process(delta):
	var transform2d = Transform2D()
	if triggered == true && shooting == false:
		on_bullet_spawned()
		shooting = true
	if shooting == true:
			...
			if bullet.alive == true:
				...
				if bullet.lifeTime <= 0:
					if bullet.spawned == false:
						bullet.spawned = true
						bullet.position = get_parent().get_node("enemy").get_transform().origin
						bullet.speed = randf_range(SPEED_MIN, SPEED_MAX)
						bullet.speedX = bullet.speed * bullet.direction.x
						bullet.speedY = bullet.speed * bullet.direction.y
